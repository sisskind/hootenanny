...
private:
  ...
  void _updateNode_Services(long id, double lat, double lon, const Tags& tags);

  void _updateRelation_Services(long id, const Tags& tags);

  void _updateWay_Services(long id, const Tags& tags);
  ...
  shared_ptr<QSqlQuery> _selectElements_Services(const long elementId, const ElementType& elementType,
                                       const long limit, const long offset);

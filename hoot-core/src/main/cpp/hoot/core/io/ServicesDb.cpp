...
void ServicesDb::endChangeset()
{
  ...
  // NOTE: do *not* alter _currChangesetId or _changesetEnvelope yet.  We haven't written data to database yet so
...
long ServicesDb::_getNextWayId_Services()
{
  const long mapId = _currMapId;
...
bool ServicesDb::insertRelation(const long relationId, const Tags &tags)
{
  ...
  case DBTYPE_OSMAPI:
    ...
    break;
  default:
    throw HootException("InsertRelation in unsupported DB type");
    break;
  }
...
shared_ptr<QSqlQuery> ServicesDb::selectAllElements(const long elementId, const ElementType& elementType)
{
  ...
  {
  case DBTYPE_SERVICES:
    return _selectElements_Services(elementId, elementType, -1, 0);
    break;
  /*
  case DBTYPE_OSMAPI:
...
shared_ptr<QSqlQuery> ServicesDb::selectAllElements(const ElementType& elementType)
{
  ...
  {
  case DBTYPE_SERVICES:
    return _selectElements_Services(-1, elementType, -1, 0);
    break;
  /*
  case DBTYPE_OSMAPI:
...
void ServicesDb::updateNode(long id, double lat, double lon,const Tags& tags)
{
  switch( _connectionType)
  {
  case DBTYPE_SERVICES:
    _updateNode_Services(id, lat, lon, tags);
    break;
  default:
    throw HootException("UpdateNode called on unsupported database type");
    break;
  }
}
...
void ServicesDb::updateRelation(const long id, const Tags& tags)
{
  ...
  {
  case DBTYPE_SERVICES:
    _updateRelation_Services(id, tags);
    break;
  case DBTYPE_OSMAPI:
...
void ServicesDb::_updateNode_Services(long id, double lat, double lon, const Tags& tags)
{
  const long mapId = _currMapId;
  const long changeSetId = _currChangesetId;
  _flushBulkInserts();
...
void ServicesDb::_updateRelation_Services(long id, const Tags& tags)
{
  const long mapId = _currMapId;
  const long changeSetId = _currChangesetId;
...
void ServicesDb::_updateWay_Services(long id, const Tags& tags)
{
  const long mapId = _currMapId;
  const long changeSetId = _currChangesetId;
